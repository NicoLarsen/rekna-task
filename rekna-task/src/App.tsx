...
import {
  ...
  fetchRecurringTasks,
  createMonthlyTasks,
  updateExistingTasksWithMissingFields,
  bulkUpdateStatuses,
  moveTaskToDone,
...
export default function App() {
  ...
  const handleCreateTasks = async () => {
    ...
    try {
      ...
      // First, update existing tasks with any missing fields (e.g., newly added VAT fields)
      const updatedCount = await updateExistingTasksWithMissingFields(hailer, companies, tasks, controller.signal);
      if (updatedCount > 0) {
        console.log(`[App] Updated ${updatedCount} existing tasks with missing fields`);
      }

      // Then create new tasks for companies without tasks
      const count = await createMonthlyTasks(hailer, companies, tasks, year, month, controller.signal);
      if (controller.signal.aborted) {
        toast({ description: `Stopped after updating ${updatedCount} and creating ${count} task(s).`, status: 'warning', duration: 3000, isClosable: true });
      } else if (count === 0 && updatedCount === 0) {
        toast({ description: 'All companies already have tasks for this month.', status: 'info', duration: 3000, isClosable: true });
      } else {
        const messages: string[] = [];
        if (count > 0) messages.push(`Created ${count} task(s)`);
        if (updatedCount > 0) messages.push(`Updated ${updatedCount} existing task(s) with new fields`);
        toast({ description: messages.join('. ') + '.', status: 'success', duration: 3000, isClosable: true });
      }
  ...
  const handleCategoryCommentChange = async (comment: string) => {
    if (!categoryPopupActivity || !categoryPopupState) return;
    try {
      await updateCategoryComment(hailer, categoryPopupActivity._id, categoryPopupState.category, comment);
      // Update local state to reflect the saved comment
      setTasks((prevTasks) =>
        prevTasks.map((t) =>
          t._id === categoryPopupActivity._id
            ? { ...t, categoryComments: { ...t.categoryComments, [categoryPopupState.category]: comment } }
            : t
        )
      );
    } catch (err) {
      toast({ description: err instanceof Error ? err.message : 'Failed to save comment', status: 'error', duration: 5000, isClosable: true });
    }
  };
